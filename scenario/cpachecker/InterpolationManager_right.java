...
public final class InterpolationManager {
    ...
    @Option(name = "incrementalCexTraceCheck", description = "use incremental search in counterexample analysis, " + "to find the minimal infeasible prefix")
    private boolean incrementalCheck = false;

    @Option(name = "cexTraceCheckDirection", description = "Direction for doing counterexample analysis: from start of trace, from end of trace, or alternatingly from start and end of the trace towards the middle")
    ...
    public InterpolationManager(FormulaManagerView pFmgr, PathFormulaManager pPmgr, Solver pSolver, FormulaManagerFactory pFmgrFactory, Configuration config, ShutdownNotifier pShutdownNotifier, LogManager pLogger) throws InvalidConfigurationException {
   ...
   */
    public CounterexampleTraceInfo buildCounterexampleTrace(final List<BooleanFormula> pFormulas, final Set<ARGState> elementsOnPath, final boolean computeInterpolants) throws CPAException, InterruptedException {
    ...
    public CounterexampleTraceInfo buildCounterexampleTrace(final List<BooleanFormula> pFormulas, final Set<ARGState> elementsOnPath) throws CPAException, InterruptedException {
        return buildCounterexampleTrace(pFormulas, elementsOnPath, true);
    }

    private CounterexampleTraceInfo buildCounterexampleTrace0(final List<BooleanFormula> pFormulas, final Set<ARGState> elementsOnPath, final boolean computeInterpolants) throws CPAException, InterruptedException {
        logger.log(Level.FINEST, "Building counterexample trace");
        cexAnalysisTimer.start();
        try {
            // Final adjustments to the list of formulas
            // copy because we will change the list
            List<BooleanFormula> f = new ArrayList<>(pFormulas);
   ...
   */
    private List<Pair<BooleanFormula, Integer>> orderFormulas(final List<BooleanFormula> traceFormulas) {
        ...
        if (direction == CexTraceAnalysisDirection.ZIGZAG) {
            ...
            final boolean backwards = direction == CexTraceAnalysisDirection.BACKWARDS;
            final int increment = backwards ? -1 : 1;
            for (int i = backwards ? traceFormulas.size() - 1 : 0; backwards ? i >= 0 : i < traceFormulas.size(); i += increment) {
                orderedFormulas.add(Pair.of(traceFormulas.get(i), i));
            }
        }
        ImmutableList<Pair<BooleanFormula, Integer>> result = orderedFormulas.build();
        assert traceFormulas.size() == result.size();
        assert ImmutableMultiset.copyOf(from(result).transform(Pair.getProjectionToFirst())).equals(ImmutableMultiset.copyOf(traceFormulas)) : "Ordered list does not contain the same formulas with the same count";
        return result;
    }
   ...
   */
    private <T> List<BooleanFormula> getInterpolants(InterpolatingProverEnvironment<T> pItpProver, List<T> itpGroupsIds) throws InterruptedException {
   ...
   * @throws InterruptedException
   */
    private <T> CounterexampleTraceInfo getErrorPath(List<BooleanFormula> f, InterpolatingProverEnvironment<T> pItpProver, Set<ARGState> elementsOnPath) throws CPATransferException, InterruptedException {
   ...
   */
     ...
     * @throws CPAException
     */
        CounterexampleTraceInfo buildCounterexampleTrace(List<BooleanFormula> f, Set<ARGState> elementsOnPath, boolean computeInterpolants) throws CPAException, InterruptedException {
     ...
     * @throws InterruptedException
     */
        private boolean checkInfeasabilityOfTrace(List<Pair<BooleanFormula, Integer>> traceFormulas, List<T> itpGroupsIds) throws InterruptedException {
            ...
            // index of first mis-matching formula in both lists
            int firstBadIndex = -1;
            while (assertedIterator.hasNext()) {
                ...
                if (todoFormula.getFirst().equals(assertedFormula.getFirst())) {
                    ...
                    @SuppressWarnings("unchecked") T itpGroupId = assertedFormula.getSecond();
            ...
            // solver stack was already empty, nothing do to
                ...
                List<Pair<BooleanFormula, T>> toDeleteFormulas = currentlyAssertedFormulas.subList(firstBadIndex, currentlyAssertedFormulas.size());
                // remove formulas from solver stack
                for (int i = 0; i < toDeleteFormulas.size(); i++) {
                    itpProver.pop();
                }
                // this removes from currentlyAssertedFormulas
            ...
            assert Iterables.elementsEqual(from(traceFormulas).transform(Pair.getProjectionToFirst()), from(currentlyAssertedFormulas).transform(Pair.getProjectionToFirst()));
            if (incrementalCheck) {
                // we did unsat checks
