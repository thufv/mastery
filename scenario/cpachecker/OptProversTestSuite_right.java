...
@RunWith(Suite.class)
@Suite.SuiteClasses({ OptProversTestSuite.Mathsat5MaximizationTest.class, OptProversTestSuite.Z3MaximizationTest.class })
public class OptProversTestSuite {

    public static class Z3MaximizationTest extends OptimizationTest {

        @Override
        public FormulaManager getFormulaManager(LogManager logger, Configuration config, ShutdownNotifier pShutdownNotifier) throws InvalidConfigurationException {
            return Z3FormulaManager.create(logger, config, pShutdownNotifier, null, 42);
        }
    }
    ...
    public static class Mathsat5MaximizationTest extends OptimizationTest {
        ...
        @Override
        public FormulaManager getFormulaManager(LogManager logger, Configuration config, ShutdownNotifier pShutdownNotifier) throws InvalidConfigurationException {
            return Mathsat5FormulaManager.create(logger, config, pShutdownNotifier, null, 42);
        }
    }

    @Ignore
    public abstract static class OptimizationTest {
        ...
        protected abstract FormulaManager getFormulaManager(LogManager logger, Configuration config, ShutdownNotifier pShutdownNotifier) throws InvalidConfigurationException;

        @Before
        public void loadLibrary() throws Exception {
            Configuration config = Configuration.builder().setOption("cpa.predicate.loadOptimathsat5", "true").build();
        ...
        public void testUnbounded() throws Exception {
            try (OptEnvironment prover = mgr.newOptEnvironment()) {
                ...
                List<BooleanFormula> constraints = ImmutableList.of(rfmgr.greaterOrEquals(x, rfmgr.makeNumber("10")), rfmgr.equal(x, obj));
        ...
        public void testUnfeasible() throws Exception {
            try (OptEnvironment prover = mgr.newOptEnvironment()) {
                ...
                List<BooleanFormula> constraints = ImmutableList.of(rfmgr.lessThan(x, y), rfmgr.greaterThan(x, y));
                prover.addConstraint(bfmgr.and(constraints));
                prover.maximize(x);
        ...
        x - y >= 1
       */
                List<BooleanFormula> constraints = ImmutableList.of(ifmgr.lessOrEquals(x, ifmgr.makeNumber(10)), ifmgr.lessOrEquals(y, ifmgr.makeNumber(15)), ifmgr.equal(obj, ifmgr.add(x, y)), ifmgr.greaterOrEquals(ifmgr.subtract(x, y), ifmgr.makeNumber(1)));
                prover.addConstraint(bfmgr.and(constraints));
                int handle = prover.maximize(obj);
                // Maximize for x.
                OptStatus response = prover.check();
        ...
        x - y >= 1
       */
                List<BooleanFormula> constraints = ImmutableList.of(rfmgr.lessOrEquals(x, rfmgr.makeNumber(10)), rfmgr.lessOrEquals(y, rfmgr.makeNumber(15)), rfmgr.equal(obj, rfmgr.add(x, y)), rfmgr.greaterOrEquals(rfmgr.subtract(x, y), rfmgr.makeNumber(1)));
                prover.addConstraint(bfmgr.and(constraints));
                OptStatus response;
                prover.push();
                int handle = prover.maximize(obj);
