...
public class ValueAnalysisRefiner extends GenericRefiner<ValueAnalysisState, ValueAnalysisInterpolant> {
    ...
    @Option(// name="refinement.basisStrategy",
    secure = true, description = "Which base precision should be used for a new precision? " + "ALL: During refinement, collect precisions from the complete ARG. " + "SUBGRAPH: During refinement, keep precision from all removed parts (subgraph) of the ARG. " + "CUTPOINT: Only the cut-point's precision is kept. " + "TARGET: Only the target state's precision is kept.")
    private /* There are usually more tracked variables at the target location that at the cut-point.
   ...
   */
    BasisStrategy basisStrategy = BasisStrategy.SUBGRAPH;

    private static enum BasisStrategy {

        ALL(), SUBGRAPH(), TARGET(), CUTPOINT()
    }
   ...
   */
    ...
    // Statisticsprivate final StatCounter rootRelocations = new StatCounter("Number of root relocations");
    ...
    public static ValueAnalysisRefiner create(final ConfigurableProgramAnalysis pCpa) throws InvalidConfigurationException {
        ...
        final StrongestPostOperator<ValueAnalysisState> strongestPostOp = new ValueAnalysisStrongestPostOperator(logger, config, cfa);
        final ValueAnalysisFeasibilityChecker checker = new ValueAnalysisFeasibilityChecker(strongestPostOp, logger, cfa, config);
        final GenericPrefixProvider<ValueAnalysisState> prefixProvider = new ValueAnalysisPrefixProvider(logger, cfa, config, valueAnalysisCpa.getShutdownNotifier());
        return new ValueAnalysisRefiner(argCpa, checker, strongestPostOp, new PathExtractor(logger, config), prefixProvider, config, logger, valueAnalysisCpa.getShutdownNotifier(), cfa);
    }
    ...
    ValueAnalysisRefiner(final ARGCPA pArgCPA, final ValueAnalysisFeasibilityChecker pFeasibilityChecker, final StrongestPostOperator<ValueAnalysisState> pStrongestPostOperator, final PathExtractor pPathExtractor, final GenericPrefixProvider<ValueAnalysisState> pPrefixProvider, final Configuration pConfig, final LogManager pLogger, final ShutdownNotifier pShutdownNotifier, final CFA pCfa) throws InvalidConfigurationException {
        super(pArgCPA, pFeasibilityChecker, new ValueAnalysisPathInterpolator(pFeasibilityChecker, pStrongestPostOperator, pPrefixProvider, pConfig, pLogger, pShutdownNotifier, pCfa), ValueAnalysisInterpolantManager.getInstance(), pPathExtractor, pConfig, pLogger);
    ...
    @Override
    protected void refineUsingInterpolants(final ARGReachedSet pReached, final InterpolationTree<ValueAnalysisState, ValueAnalysisInterpolant> pInterpolationTree) throws InterruptedException {
        ...
        for (ARGState root : refinementRoots) {
            ...
            if (refinementRoots.size() == 1 && isSimilarRepeatedRefinement(pInterpolationTree.extractPrecisionIncrement(root).values())) {
            ...
            switch(basisStrategy) {
                case ALL:
                    basePrecision = mergeValuePrecisionsForSubgraph((ARGState) reached.getFirstState(), reached);
                    break;
                case SUBGRAPH:
    ...
    private VariableTrackingPrecision mergeValuePrecisionsForSubgraph(final ARGState pRefinementRoot, final UnmodifiableReachedSet pReached) {
        // get all unique precisions from the subtree
        Set<VariableTrackingPrecision> uniquePrecisions = Sets.newIdentityHashSet();
        for (ARGState descendant : from(pRefinementRoot.getSubgraph()).filter(not(ARGState::isCovered))) {
   ...
   */
    ...
    private ARGState relocateRefinementRoot(final ARGState pRefinementRoot, final boolean predicatePrecisionIsAvailable) throws InterruptedException {
   ...
   */
    ...
    protected void printAdditionalStatistics(PrintStream pOut, Result pResult, UnmodifiableReachedSet pReached) {
        ...
        writer.put(rootRelocations).put(repeatedRefinements).put("Number of unique precision increments", previousRefinementIds.size());
    }
}
