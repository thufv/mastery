...
public class CoreComponentsFactory {
    ...
    public static enum SpecAutomatonCompositionType {
        ...
        NONE(), TARGET_SPEC(), BACKWARD_TO_ENTRY_SPEC()
    }
    ...
    @Option(secure = true, name = "restartAlgorithmWithARGReplay", description = "run a sequence of analysis, where the previous ARG is inserted into the current ARGReplayCPA.")
    ...
    private boolean analysisNeedsShutdownManager() {
        ...
        return !useProofCheckAlgorithm && !useRestartingAlgorithm && !useImpactAlgorithm && !useRestartAlgorithmWithARGReplay && !runCBMCasExternalTool && useBMC;
    }

    public Algorithm createAlgorithm(final ConfigurableProgramAnalysis cpa, final String programDenotation, final CFA cfa) throws InvalidConfigurationException, CPAException {
        ...
        if (useProofCheckAlgorithm) {
            ...
            algorithm = ExceptionHandlingAlgorithm.create(config, algorithm, cpa, logger, shutdownNotifier, checkCounterexamples, useCEGAR);
    ...
    public ConfigurableProgramAnalysis createCPA(final CFA cfa, SpecAutomatonCompositionType composeWithSpecificationCPAs) throws InvalidConfigurationException, CPAException {
