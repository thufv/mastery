...
class ASTConverter {
    ...
    // Calls to this functions are handled by this class and replaced with regular C code.private static final String FUNC_CONSTANT = "__builtin_constant_p";
    ...
    // they are in the same run) uniqueprivate static int anonTypeCounter = 0;
    ...
    public ASTConverter(EclipseCParserOptions pOptions, Scope pScope, LogManagerWithoutDuplicates pLogger, ParseContext pParseContext, MachineModel pMachineModel, String pStaticVariablePrefix, Sideassignments pSideAssignmentStack) {
    ...
    public CExpression convertExpressionWithoutSideEffects(IASTExpression e) {
        ...
        } else if (e instanceof IASTUnaryExpression && (((IASTUnaryExpression) e).getOperator() == IASTUnaryExpression.op_postFixDecr || ((IASTUnaryExpression) e).getOperator() == IASTUnaryExpression.op_postFixIncr)) {
            return addSideAssignmentsForUnaryExpressions(((CAssignment) node).getLeftHandSide(), node.getFileLocation(), typeConverter.convert(e.getExpressionType()), ((CBinaryExpression) ((CAssignment) node).getRightHandSide()).getOperator());
   ...
   */
    ...
    private CExpression addSideassignmentsForExpressionsWithoutSideEffects(CAstNode node, IASTExpression e) {
        ...
        if (e.getExpressionType() instanceof IProblemType) {
            tmp = createInitializedTemporaryVariable(getLocation(e), ((CRightHandSide) node).getExpressionType(), (CInitializer) null);
        } else {
            tmp = createTemporaryVariable(e);
        }
        sideAssignmentStack.addPreSideAssignment(new CFunctionCallAssignmentStatement(getLocation(e), tmp, (CFunctionCallExpression) node));
        return tmp;
    }
   ...
   * @param op binary operator, should be PLUS or MINUS */
    private CIdExpression addSideAssignmentsForUnaryExpressions(final CLeftHandSide exp, final FileLocation fileLoc, final CType type, final BinaryOperator op) {
    ...
    protected CAstNode convertExpressionWithSideEffects(IASTExpression e) {
        ...
        if (converted == null || !options.simplifyConstExpressions() || !(converted instanceof CExpression)) {
            return converted;
        }
    ...
    private CAstNode convertExpressionWithSideEffectsNotSimplified(IASTExpression e) {
        ...
        if (e == null) {
            ...
            if (type instanceof CFunctionType && !(isFunctionCallNameExpression(e) || isAddressOfArgument(e))) {
                exp = new CUnaryExpression(exp.getFileLocation(), new CPointerType(type.isConst(), type.isVolatile(), type), exp, UnaryOperator.AMPER);
            }
            return exp;
    ...
    private boolean isAddressOfArgument(IASTExpression e) {
        return e.getParent() instanceof IASTUnaryExpression && ((IASTUnaryExpression) e.getParent()).getOperator() == IASTUnaryExpression.op_amper;
    }

    static enum CONDITION {

        NORMAL(), ALWAYS_FALSE(), ALWAYS_TRUE()
    }
   ...
   * the rest works as with the condition kind method for CExpressions */
    private CONDITION getConditionKind(IASTExpression exp) {
        if (exp instanceof IASTBinaryExpression && (((IASTBinaryExpression) exp).getOperator() == IASTBinaryExpression.op_logicalAnd || ((IASTBinaryExpression) exp).getOperator() == IASTBinaryExpression.op_logicalOr)) {
   ...
   */
    private CType convertType(IASTExpression e) {
        ...
        // workaround for strange CDT behaviour
            ...
            return typeConverter.convert(((IASTConditionalExpression) e).getNegativeResultExpression().getExpressionType());
        }
        return type;
    }

    private CIdExpression createInitializedTemporaryVariable(final FileLocation loc, final CType pType, @Nullable CExpression initializer) {
        return createInitializedTemporaryVariable(loc, pType, initializer == null ? null : new CInitializerExpression(loc, initializer));
    }
   ...
   */
    private CIdExpression createInitializedTemporaryVariable(final FileLocation loc, final CType pType, @Nullable CInitializer initializer) {
        ...
        CVariableDeclaration decl = new CVariableDeclaration(loc, scope.isGlobalScope(), CStorageClass.AUTO, type, name, name, scope.createScopedNameOf(name), initializer);
    ...
    private CAstNode convert(IASTBinaryExpression e) {
        ...
        if (isAssign) {
            if (!(leftHandSide instanceof CLeftHandSide)) {
                throw parseContext.parseError("Lefthandside of Assignment " + e.getRawSignature() + " is no CLeftHandside but should be.", leftHandSide);
            }
            CLeftHandSide lhs = (CLeftHandSide) leftHandSide;
            if (op == null) {
                // a = b
                // right-hand side may have a function call
                CAstNode rightHandSide = convertExpressionWithSideEffects(e.getOperand2());
    ...
    private CBinaryExpression buildBinaryExpression(CExpression operand1, CExpression operand2, BinaryOperator op) {
        try {
    ...
    private static boolean isPointerToVoid(final IASTExpression e) {
        return (e.getExpressionType() instanceof IPointerType) && ((IPointerType) e.getExpressionType()).getType() instanceof IBasicType && ((IBasicType) ((IPointerType) e.getExpressionType()).getType()).getKind() == Kind.eVoid;
    }

    private static boolean isRightHandSide(final IASTExpression e) {
        return e.getParent() instanceof IASTBinaryExpression && ((IASTBinaryExpression) e.getParent()).getOperator() == IASTBinaryExpression.op_assign && ((IASTBinaryExpression) e.getParent()).getOperand2() == e;
    }
     ...
     */
        ...
        if (e.getOperand() instanceof CASTFunctionCallExpression && castType.getCanonicalType() instanceof CPointerType && isRightHandSide(e) && isPointerToVoid(e.getOperand())) {
            return convertExpressionWithSideEffects(e.getOperand());
        } else {
    ...
    private CFieldReference convert(IASTFieldReference e) {
        ...
        if (ownerType instanceof CElaboratedType) {
            // otherwise getCanonicalType is broken
            assert ((CElaboratedType) ownerType).getRealType() == null;
            throw parseContext.parseError("Cannot access the field " + fieldName + " in type " + ownerType + " which does not have a definition", e);
        } else if (ownerType instanceof CProblemType) {
            fullFieldReference = new CFieldReference(loc, typeConverter.convert(e.getExpressionType()), fieldName, owner, e.isPointerDereference());
        } else if (ownerType instanceof CCompositeType) {
            wayToInnerField = getWayToInnerField((CCompositeType) ownerType, fieldName, loc, new ArrayList<>());
            if (!wayToInnerField.isEmpty()) {
                ...
                throw parseContext.parseError("Accessing unknown field " + fieldName + " in type " + ownerType, e);
            }
        } else {
            throw parseContext.parseError("Cannot access field " + fieldName + " in type " + ownerType + " which is not a composite type", e);
        }
        // but only if we are not in global scope, otherwise there will be parsing errors
        if (options.simplifyPointerExpressions() && (wayToInnerField.size() > 1 || owner instanceof CFieldReference) && !scope.isGlobalScope()) {
   ...
   */
    ...
    private CRightHandSide convert(IASTFunctionCallExpression e) {
        ...
        if (functionName instanceof CIdExpression) {
            ...
            if (((CIdExpression) functionName).getName().equals(FUNC_CONSTANT) && params.size() == 1 && scope.lookupFunction(FUNC_CONSTANT) == null) {
            ...
            if ((declaration == null) && FUNC_EXPECT.equals(((CIdExpression) functionName).getName()) && params.size() == 2) {
        ...
        if (functionNameType instanceof CPointerType && ((CPointerType) functionNameType).getType() instanceof CFunctionType) {
        ...
        if (containsProblemType(returnType)) {
            ...
            if (declaration != null) {
                ...
                logger.log(Level.FINE, loc + ":", "Replacing return type", returnType, "of function call", e.getRawSignature(), "with", returnType);
            } else {
        ...
        if (declaration == null && functionName instanceof CIdExpression && returnType instanceof CVoidType) {
            ...
            logger.log(Level.FINE, loc + ":", "Setting return type of of undeclared function", functionName, "to int.");
    ...
    private CIdExpression convert(IASTIdExpression e) {
        ...
        if (declaration instanceof CEnumerator && type instanceof CElaboratedType && ((CElaboratedType) type).getKind() == ComplexTypeKind.ENUM && ((CElaboratedType) type).getRealType() == null) {
            ...
            if (enumType != null) {
                type = new CElaboratedType(type.isConst(), type.isVolatile(), ComplexTypeKind.ENUM, enumType.getName(), enumType.getOrigName(), enumType);
            }
        }
            ...
            case IASTUnaryExpression.op_star:
                {
                    ...
                    if (operandType instanceof CPointerType) {
                        ...
                        if (!(operandType instanceof CProblemType)) {
                            logger.logf(Level.WARNING, "%s: Dereferencing of non-pointer type %s in expression %s", fileLoc, operandType, e.getRawSignature());
                        }
                        type = typeConverter.convert(e.getExpressionType());
                    }
    ...
    private CExpression simplifyUnaryPointerExpression(final CExpression operand, final FileLocation fileLoc, final CType type) {
        // FOLLOWING IF CLAUSE WILL ONLY BE EVALUATED WHEN THE OPTION cfa.simplifyPointerExpressions IS SET TO TRUE
        if (options.simplifyPointerExpressions()) {
            ...
            } else // in case of *(a[index])
            if (operand instanceof CArraySubscriptExpression) {
                CIdExpression tmpVar = createInitializedTemporaryVariable(fileLoc, operandType, operand);
                return new CPointerExpression(fileLoc, type, tmpVar);
            } else // in case of *& both can be left out
            if (operand instanceof CUnaryExpression && ((CUnaryExpression) operand).getOperator() == UnaryOperator.AMPER) {
                return ((CUnaryExpression) operand).getOperand();
            } else // in case of ** a temporary variable is needed
            if (operand instanceof CPointerExpression) {
                CIdExpression tmpVar = createInitializedTemporaryVariable(fileLoc, operandType, operand);
                return new CPointerExpression(fileLoc, type, tmpVar);
            } else // in case of p.e. *(a+b) or *(a-b) or *(a ANY_OTHER_OPERATOR b) a temporary variable is needed
    ...
    private CTypeIdExpression convert(IASTTypeIdExpression e) {
        ...
        if (typeIdOperator == TypeIdOperator.ALIGNOF || typeIdOperator == TypeIdOperator.SIZEOF) {
            ...
            if (typeId.isIncomplete()) {
                ...
                throw parseContext.parseError("Invalid application of " + typeIdOperator.getOperator() + " to incomplete type " + typeId, e);
            }
        } else {
    ...
    public CReturnStatement convert(final IASTReturnStatement s) {
        ...
        if (returnVariableDeclaration.isPresent()) {
            ...
            if (rhs != null) {
                returnAssignment = Optional.<CAssignment>of(new CExpressionAssignmentStatement(loc, lhs, rhs));
            } else {
    ...
    private CDeclaration createDeclaration(FileLocation fileLoc, CStorageClass cStorageClass, CType type, IASTDeclarator d) {
        ...
        if (d != null) {
            ...
            CVariableDeclaration declaration = new CVariableDeclaration(fileLoc, isGlobal, cStorageClass, type, name, origName, scope.createScopedNameOf(name), null);
    ...
    private Triple<CType, IASTInitializer, String> convert(IASTDeclarator d, CType specifier) {
        while (d != null && d.getClass() == org.eclipse.cdt.internal.core.dom.parser.c.CASTDeclarator.class && d.getPointerOperators().length == 0 && d.getAttributes().length == 0 && d.getAttributeSpecifiers().length == 0 && d.getInitializer() == null && d.getNestedDeclarator() != null) {
        ...
        if (d instanceof IASTFunctionDeclarator) {
            ...
            // there may be no name at all, for example in parameter declarations
            ...
            if (type instanceof CArrayType) {
                ...
                if (arrayType.getLength() == null && initializer instanceof IASTEqualsInitializer) {
                    ...
                    if (initClause instanceof IASTInitializerList) {
                        ...
                        if (length != -1) {
                            CExpression lengthExp = new CIntegerLiteralExpression(getLocation(initializer), CNumericTypes.INT, BigInteger.valueOf(length));
                            type = new CArrayType(arrayType.isConst(), arrayType.isVolatile(), arrayType.getType(), lengthExp);
                        }
                    } else {
                        // will be converted as char a[4] = "abc";
                        if (initClause instanceof CASTLiteralExpression && (arrayType.getType().equals(CNumericTypes.CHAR) || arrayType.getType().equals(CNumericTypes.SIGNED_CHAR) || arrayType.getType().equals(CNumericTypes.UNSIGNED_CHAR))) {
    ...
    private CInitializer convert(IASTInitializerClause i, @Nullable CVariableDeclaration declaration) {
        if (i instanceof IASTExpression) {
            ...
            throw parseContext.parseError("unknown initializer claus: " + i.getClass().getSimpleName(), i);
        }
    }
    ...
    private CInitializer convert(org.eclipse.cdt.core.dom.ast.c.ICASTDesignatedInitializer init, @Nullable CVariableDeclaration declaration) {
        ...
        for (ICASTDesignator designator : desInit) {
            ...
            if (designator instanceof ICASTFieldDesignator) {
                ...
                r = new CArrayDesignator(fileLoc, convertExpressionWithoutSideEffects(((ICASTArrayDesignator) designator).getSubscriptExpression()));
            } else if (designator instanceof IGCCASTArrayRangeDesignator) {
                r = new CArrayRangeDesignator(fileLoc, convertExpressionWithoutSideEffects(((IGCCASTArrayRangeDesignator) designator).getRangeFloor()), convertExpressionWithoutSideEffects(((IGCCASTArrayRangeDesignator) designator).getRangeCeiling()));
            } else {
        ...
        if (ic instanceof IASTExpression) {
            ...
            if (initializer instanceof CAssignment) {
                ...
                if (declaration != null) {
                    ...
                    sideAssignmentStack.addPostSideAssignment(new CFunctionCallAssignmentStatement(loc, var, (CFunctionCallExpression) initializer));
                    // empty initializer
                    return null;
                } else {
                    ...
                    sideAssignmentStack.addPreSideAssignment(new CFunctionCallAssignmentStatement(loc, var, (CFunctionCallExpression) initializer));
                    return new CInitializerExpression(loc, var);
                }
            }
            if (!(initializer instanceof CExpression)) {
                throw parseContext.parseError("Initializer is not free of side-effects, it is a " + initializer.getClass().getSimpleName(), e);
            }
    ...
    static String convert(IASTName n) {
        // TODO verify toString() is the correct method
        return n.toString();
    }
