...
public class ErrorUtils {
    ...
    static {
        ERROR_CLASS.initialize(true, Collections.<TypeParameterDescriptor>emptyList(), Collections.<JetType>emptyList(), getErrorScope(), ERROR_FUNCTION_GROUP, ERROR_CONSTRUCTOR);
    }
    ...
    private static final VariableDescriptor ERROR_PROPERTY = new PropertyDescriptor(ERROR_CLASS, Collections.<AnnotationDescriptor>emptyList(), Modality.OPEN, Visibility.INTERNAL, true, null, "<ERROR PROPERTY>", ERROR_PROPERTY_TYPE, ERROR_PROPERTY_TYPE);
    ...
    private static FunctionDescriptor createErrorFunction(List<TypeParameterDescriptor> typeParameters, List<JetType> positionedValueArgumentTypes) {
        ...
        return functionDescriptor.initialize(null, typeParameters, getValueParameters(functionDescriptor, positionedValueArgumentTypes), createErrorType("<ERROR FUNCTION RETURN>"), Modality.OPEN, Visibility.INTERNAL);
    }

    public static FunctionDescriptor createErrorFunction(int typeParameterCount, List<JetType> positionedValueParameterTypes) {
        return new FunctionDescriptorImpl(ERROR_CLASS, Collections.<AnnotationDescriptor>emptyList(), "<ERROR FUNCTION>").initialize(null, // TODO
        Collections.<TypeParameterDescriptor>emptyList(), // TODO
        Collections.<ValueParameterDescriptor>emptyList(), createErrorType("<ERROR FUNCTION RETURN TYPE>"), Modality.OPEN, Visibility.INTERNAL);
    }
    ...
    private static List<ValueParameterDescriptor> getValueParameters(FunctionDescriptor functionDescriptor, List<JetType> argumentTypes) {
        ...
        for (int i = 0, argumentTypesSize = argumentTypes.size(); i < argumentTypesSize; i++) {
            result.add(new ValueParameterDescriptorImpl(functionDescriptor, i, Collections.<AnnotationDescriptor>emptyList(), "<ERROR VALUE_PARAMETER>", ERROR_PARAMETER_TYPE, ERROR_PARAMETER_TYPE, false, false));
        }
        return result;
    }
    ...
    public static boolean isErrorType(@NotNull JetType type) {
        return type != JetTypeInferrer.NO_EXPECTED_TYPE && ((type instanceof DeferredType && ((DeferredType) type).getActualType() == null) || type instanceof ErrorTypeImpl || isError(type.getConstructor()));
    }
