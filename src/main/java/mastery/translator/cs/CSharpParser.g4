/*
Modified CSharpPaser.g4 v1.0:
1)Simplified the recursive level by removing and uniting some Expression grammar
2)The accepted capability of grammar is likely equal to the original CSharpPaser.g4
3)Use recursive nodes open rather than in just a node
*/
...
// relational_expression
	| expression (('<' | '>' | '<=' | '>=') expression | IS isType | AS type)
// equality_expression
	| expression (OP_EQ | OP_NE) expression
// and_expression
	| expression '&' expression
...
// inside: bracket_expression is replaced by ('?'? '[' indexer_argument ( ',' indexer_argument)* ']')
	: pe=primary_expression_start ('?'? '[' indexer_argument ( ',' indexer_argument)* ']')*
	  ((member_access | method_invocation | '++' | '--' | '->' identifier) ('?'? '[' indexer_argument ( ',' indexer_argument)* ']')*)*
	| '+' unary_expression
	| '-' unary_expression
